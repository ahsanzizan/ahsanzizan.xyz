...
export default withSessionRoute(async function uploadHandler(req, res) {
    ...
    try {
        if (req.method === 'POST') {
            ...
            delete req.body._id;
            const insert = await connectDB.db('personal-blog').collection('blog-post').updateOne({ _id: id }, { $set: !isNew ? req.body : { ...req.body, clicks: 0, publishDate: new Date().getTime() } }, { upsert: true });

...
export default withSessionRoute(async function uploadHandler(req, res) {
    ...
    try {
        if (req.method === 'POST') {
            ...
            delete req.body._id;
            const insert = await connectDB.db('personal-blog').collection('blog-post').updateOne({ _id: id }, { $set: !isNew ? req.body : { ...req.body, authorName: req.session.name, clickCount: 0, pubDate: new Date().getTime() } }, { upsert: true });

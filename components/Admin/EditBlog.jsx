...
export default class EditBlog extends React.Component {
    ...
    removeTag(e) {
        ...
        this.setState({
            ...
            tags: (
                this.state.data.tags.length ? this.state.data.tags.map((tag, i) => {
                    return <div key={i} className="cursor-pointer border-2 border-black px-3 py-1 bg-black font-semibold mr-3 uppercase duration-300 text-white hover:text-black hover:bg-white" onClick={this.removeTag}>{tag}</div>
                }) : null
            ),
            iTag: ""
        });

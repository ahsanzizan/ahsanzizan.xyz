...
export function VisitWorkButton({
  ...
  return (
    <a
      href={href}
      className={`group mb-12 inline-flex items-center gap-1 rounded-full border border-white px-4 py-2 text-sm transition-all duration-500 hover:bg-white hover:text-black hover:drop-shadow-glow md:px-[22px] md:py-[10px] md:text-lg ${className}`}
    >
      {children}{" "}
      <svg
        width="40"
...
export function MoreLinkButton({ href, className, children }: LinkButtonProps) {
  return (
    <Link
      href={href}
...
export function BackButton({ className }: LinkButtonProps) {
  const router = useRouter();

  return (
    <button
      onClick={() => router.back()}
      className={`group inline-flex items-center gap-2 rounded-full border border-white px-5 py-2 text-base transition-all duration-500 hover:bg-white hover:text-black hover:drop-shadow-glow md:px-[22px] md:py-[10px] md:text-lg ${className}`}
    >
      <svg
        width="16"
        height="16"
